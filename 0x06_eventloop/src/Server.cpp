...
Server::Server(EventLoop *lp) : loop(lp)
{
    ...
    Channel *servCh = new Channel(loop, serv_sock->getFd());
    std::function<void()> cb = std::bind(&Server::newConnection, this, serv_sock);
    servCh->setCallback(cb);
    servCh->enableReading();
}
Server::Server(EventLoop *lp, InetAddress addr) : loop(lp)
{
    Socket *serv_sock = new Socket;
    serv_sock->bind(addr);
    serv_sock->listen();
